#!/bin/bash
output_file="swarm_zero_to_one.csv"
...
while [ $SECONDS -lt $end_time ]; do
  ...
  output=$(python3 zero-to-one-value.py && python3 zero-to-one-value.py && python3 zero-to-one-value.py && sleep 10 && python3 zero-to-one-value.py)
